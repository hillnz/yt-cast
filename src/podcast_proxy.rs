...
impl PodcastProxy {
    ...
    pub async fn get_feed(&self, media_base_url: &str, channel_name: &str) -> Result<String, PodcastError> {
        let yt = ytdl::YtDl::new(&self.cache);
        
        let channel = yt.get_channel_info(channel_name).await
            .map_err(|e| {
                println!("{:?}", e);
                match e {
        ...
        for vid in vids {
            ...
            // Reformat date
            let raw_date = NaiveDate::parse_from_str(&vid.upload_date, "%Y%m%d")
                .context("bad upload date")?;
            ...
            if (Utc::now() - date) < Duration::days(ITEM_DELAY) {
                log::info!("Ignoring video {} which hasn't been out for {} days yet", vid.id, ITEM_DELAY);
                continue;
            }
        ...
        let rss_channel = rss_channel_builder
            .build().map_err(|e| anyhow!(e))?;
    ...
    pub async fn get_video(&self, video_id: &str) -> Result<PathBuf, PodcastError> {
        ...
        let out_path = self.cache.get_path(vec!["media", video_id], Some("mp4")).await?;
        ...
        if ! out_path.exists().await {
            ...
            let yt = ytdl::YtDl::new(&self.cache);
            yt.download_video(video_id, &out_path).await
                .map_err(|e| match e {
                        YtDlError::ItemNotFound => PodcastError::NotFound,
                        _ => PodcastError::YoutubeError(e)
                    })?;
